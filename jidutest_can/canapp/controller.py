...
from jidutest_can.can.tools.e2e import e2e_crc_data
...
from jidutest_can.can import PCANFD_500000_5000000
...
from jidutest_can.cantools import BusConfig
...
class CanController(object):
    ...
    def connect(self) -> bool:
        ...
        返回值：True/False
        """
        if self.bus and self.__connected:
            return True
    ...
    def __set_data(self, a_can):
        """
        待使用回调函数修改数据时调试
        功能说明：
            用来注册总线上在发送报文之前修改can报文数据的事件方法，
            当总线发送报文之前会先调用此方法，并对报文数据做些处理后再发送出去
        参数说明：
            :param a_can: 回调函数读取到的总线上原始的报文信息
        异常说明：无
        返回值：None
        """
        can_raw = a_can.contents
        can_id = can_raw.FIdentifier
        can_data = can_raw.FData
        logger.info(f"Update can raw data before e2e:{can_data}")
        message = self.db.get_message_by_frame_id(can_id)
        sgn_dict = message.decode(can_data)
        if self.__sending_messages.count(message) > 1:
            if self.__sending_messages.count(message) == 2:
                logger.info(f"Listened sending message: {message}")
                self.__sending_messages.append(message)
            if can_data not in self.__sending_raw_datas:
                self.__sending_raw_datas.append(can_data)
                message_name = message.name
                self.sending_dict_datas.setdefault(message_name, [])
                self.sending_dict_datas[message_name].append(sgn_dict)
                from jidutest_can.can.interfaces.tosun.define_tosun import tosun_convert_msg
                fr_msg = tosun_convert_msg(can_raw)
                logger.info(fr_msg)
        else:
            self.__sending_messages.append(message)
        logger.info(f"Update can dict data before e2e:{sgn_dict}")
        sgn_dict.update(self.__modified_data.get(message.name, {}))
        sgn_dict = self.__update_signals_with_e2e(message, sgn_dict)
        logger.info(f"Update can raw dict after e2e:{sgn_dict}")
        self.bus.can_data = message.encode(data=sgn_dict)
