...
from jidutest_can.can.tools import CanData
from jidutest_can.can.tools import Channel


class RawMessage:  # pylint: disable=too-many-instance-attributes; OK for a dataclass
    """
    The :class:`~can.RawMessage` object is used to represent CAN messages for
    ...
    may introduce unexpected behaviour. See also :meth:`~can.RawMessage.equals`.
    ...
    def __repr__(self) -> str:
        ...
        return f"can.RawMessage({', '.join(args)})"
    ...
    def __deepcopy__(self, memo: dict) -> "RawMessage":
        return RawMessage(
            timestamp=self.timestamp,
            arbitration_id=self.arbitration_id,
            is_extended_id=self.is_extended_id,
