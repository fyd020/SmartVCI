...
from jidutest_can.can.interfaces.socketcan.constants import *
from jidutest_can.can.interfaces.socketcan.utils import pack_filters
...
# The fields definition is taken from the C struct definitions in <linux/can/bcm.h>
...
def build_can_frame(msg: RawMessage) -> bytes:
...
def send_bcm(bcm_socket: socket.socket, data: bytes) -> int:
    ...
    try:
        ...
        raise CanOperationError(base + specific_message, error.errno) from error


def _compose_arbitration_id(message: RawMessage) -> int:
...
class CyclicSendTask(
    ...
    def _tx_setup(self, messages: Sequence[RawMessage]) -> None:
    ...
    def _check_bcm_task(self) -> None:
        ...
        try:
            ...
            if error.errno != errno.EINVAL:
                raise CanOperationError("failed to check", error.errno) from error
        else:
    ...
    def modify_data(self, messages: Union[Sequence[RawMessage], RawMessage]) -> None:
        ...
        .. note:: The messages must all have the same
                  :attr:`~can.RawMessage.arbitration_id` like the first message.

        :param messages:
            The messages with the new :attr:`can.RawMessage.data`.
        """
...
def capture_message(
    ...
    try:
        ...
        raise CanOperationError(f"Error receiving: {error.strerror}", error.errno)
...
class SocketcanBus(BusABC):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param channel:
            ...
            channel using :attr:`can.RawMessage.channel`.
    ...
    def send(self, msg: RawMessage, timeout: Optional[float] = None) -> None:
        ...
        raise CanOperationError("Transmit buffer full")

    def _send_once(self, data: bytes, channel: Optional[str] = None) -> int:
        try:
            if self.channel == "" and channel:
                # RawMessage must be addressed to a specific channel
...
if __name__ == "__main__":
    ...
    def sender(event: threading.Event) -> None:
        ...
        msg = RawMessage(arbitration_id=0x01, data=b"\x01\x02\x03")
