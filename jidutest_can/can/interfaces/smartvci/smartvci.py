...
from jidutest_can.can.interfaces.smartvci.basic import (
    FEATURE_FD_CAPABLE,
    IS_LINUX,
    IS_WINDOWS,
...
from jidutest_can.can.util import check_or_adjust_timing_clock
...
class SmartVCIBus(BusABC):
    def __init__(
        ...
        **kwargs: Any,
        ...
        On top of the usual :class:`~can.CanBus` methods provided,
        ...
        :param bool fd:
            Should the CanBus be initialized in CAN-FD mode.
        ...
        """
        self.m_objPCANBasic = SmartVCIBasic()
        ...
        if not str(channel).startswith("PCAN_USBBUS"):
            channel = "PCAN_USBBUS" + str(channel)
    ...
    def _get_formatted_error(self, error):
        ...
        return f"[{self.channel_info}][{time.time()}]{complete_text}"
    ...
    def reset(self):
        ...
        """
        logger.info(f"[{self.channel_info}][{time.time()}]Resetting can bus ...")
        status = self.m_objPCANBasic.Reset(self.m_PcanHandle)
        logger.info(f"[{self.channel_info}][{time.time()}]Reset can bus done.")
        return status == PCAN_ERROR_OK
    ...
    def set_device_number(self, device_number):
        ...
        try:
            ...
            logger.error("Invalid value '%s' for device number.", device_number)
            return False
        return True
    ...
    def _recv_internal(
        ...
        while True:
            ...
            if result == PCAN_ERROR_QRCVEMPTY:
                ...
                logger.warning(self._get_formatted_error(result))

            else:
                error_msg = self._get_formatted_error(result)
                logger.warning(error_msg)
                setattr(self, "_error_num", getattr(self, "_error_num", 0) + 1)
                error_num = getattr(self, "_error_num")
                import pathlib
                pcan_error_log = pathlib.Path("./pcan_error.log")
                pcan_error_log.touch()
                with open(pcan_error_log, "a") as f:
                    f.write(error_msg + "\n")
                    if not error_num % 3:
                        f.write(f"[{self.channel_info}][{time.time()}]Resetting can bus ...\n")
                        self.reset()
                        f.write(f"[{self.channel_info}][{time.time()}]Reset can bus done.\n")
                    elif error_num > 30:
                        logger.error(f"error msg num:{error_num} > 30.")
                        f.write(f"error msg num:{error_num} > 30.\n")
                        raise PcanCanOperationError(error_msg)
        ...
        # 本处增加如下判断和处理，是为了防止时间戳出现大于当前时间的问题
        if boottimeEpoch and timestamp > time.time():
            timestamp -= boottimeEpoch
        rx_msg = RawMessage(
            timestamp=timestamp,
    ...
    def send(self, msg, timeout=None) -> int:
        msgType = (
        ...
        if self.fd:
            ...
            # logger.debug("Data: %s", msg.data)
            # logger.debug("Type: %s", type(msg.data))
