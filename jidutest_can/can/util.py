...
from jidutest_can.can.tools.typechecking import BitTimingDict
from jidutest_can.can.tools.typechecking import BitTimingFdDict
...
from jidutest_can.can.tools import AcceptedIOType
from jidutest_can.can.tools import BusConfig


logger = logging.getLogger("jidutest_can.util")
...
def load_file_config(
    path: Optional[AcceptedIOType] = None, section: str = "default"
) -> Dict[str, str]:
    """
...
def load_config(
    path: Optional[AcceptedIOType] = None,
    ...
    logger.debug("can config: %s", bus_config)
    return bus_config


def _create_bus_config(config: Dict[str, Any]) -> BusConfig:
    ...
    if config.get("timing", None) is None:
        try:
            if set(BitTimingFdDict.__annotations__).issubset(config):
                config["timing"] = BitTimingFd(
                    **{
                        key: int(config[key])
                        for key in BitTimingFdDict.__annotations__
                    }
                )
            elif set(BitTimingDict.__annotations__).issubset(config):
                config["timing"] = BitTiming(
                    **{
                        key: int(config[key])
                        for key in BitTimingDict.__annotations__
                    }
                )
...
def set_logging_level(level_name: str) -> None:
    ...
    logger.debug("Logging set to %s", level_name)
