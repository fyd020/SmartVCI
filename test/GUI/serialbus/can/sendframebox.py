...
from jidutest_can import RawMessage
...
class SendFrameBox(QGroupBox):
    ...
    def _send(self):
        ...
        frame = RawMessage(arbitration_id=frame_id,
                           is_rx=False,
                           is_remote_frame=self.m_ui.remoteFrame.isChecked(),
                           is_fd=self.m_ui.flexibleDataRateBox.isChecked(),
                           is_extended_id=self.m_ui.extendedFormatBox.isChecked(),
                           data=payload,
                           bitrate_switch=self.m_ui.bitrateSwitchBox.isChecked(),
                           is_error_frame=self.m_ui.errorFrame.isChecked()
                           )
